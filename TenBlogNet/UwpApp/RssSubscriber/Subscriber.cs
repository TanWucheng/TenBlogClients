...
namespace UwpApp.RssSubscriber
{
    ...
    public static class Subscriber
    {
        ...
        /// <returns></returns>
        public static async Task<Feed> Subscribe(string rssUrl, int articleCount = int.MaxValue,
            bool doHttpRequest = false)
        {
            ...
            {
                var xml = await ReadCacheRssXmlAsync();
        ...
        /// </summary>
        /// <returns></returns>
        private static async Task<string> ReadCacheRssXmlAsync()
        {
            var localCacheFolderFolder = ApplicationData.Current.LocalCacheFolder;
            try
            {
                var storageFile = await localCacheFolderFolder.GetFileAsync(RssXmlFileName);
                var xml = await FileIO.ReadTextAsync(storageFile);
                return xml;
            }
        ...
        private static async void WriteCacheRssXmlAsync(string xml)
        {
            var localCacheFolderFolder = ApplicationData.Current.LocalCacheFolder;
            var storageFile =
                await localCacheFolderFolder.CreateFileAsync(RssXmlFileName, CreationCollisionOption.ReplaceExisting);
            await FileIO.WriteTextAsync(storageFile, xml);
        }
